...
func prepareOptions(ProvideOptions []ProvideOption, RequestOptions []RequestOption) (
...
func AcquireLease(
	...
	log.Infof("%v: acquiring lease", clientID)
	...
	go func() {
		errCh <- ns.WithNetNSPath(netns, func(_ ns.NetNS) error {
			...
			log.Infof("%v: lease acquired: [%s], expiration is %v", l.clientID, ipString, l.expireTime)
...
func (l *DHCPLease) acquire() error {
	...
	if (l.link.Attrs().Flags & net.FlagUp) != net.FlagUp {
		log.Infof("Link %q down. Attempting to set up", l.link.Attrs().Name)
...
func (l *DHCPLease) maintain() {
	...
	for {
		...
		case leaseStateBound:
			...
			if sleepDur <= 0 {
				log.Infof("%v: renewing lease", l.clientID)
		...
		case leaseStateRenewing:
			if err := l.renew(); err != nil {
				log.Infof("%v: %v", l.clientID, err)

				if time.Now().After(l.rebindingTime) {
					log.Infof("%v: renewal time expired, rebinding", l.clientID)
					state = leaseStateRebinding
				}
			} else {
				log.Infof("%v: lease renewed, expiration is %v", l.clientID, l.expireTime)
				state = leaseStateBound
			}
		...
		case leaseStateRebinding:
			if err := l.acquire(); err != nil {
				...
				if time.Now().After(l.expireTime) {
					log.Infof("%v: lease expired, bringing interface DOWN", l.clientID)
					l.downIface()
					return
				}
			} else {
				log.Infof("%v: lease rebound, expiration is %v", l.clientID, l.expireTime)
				state = leaseStateBound
			}
		}
		...
		case <-l.stop:
			if err := l.release(); err != nil {
				log.Infof("%v: failed to release DHCP lease: %v", l.clientID, err)
			}
			return
		}
	}
...
func (l *DHCPLease) downIface() {
	...
	//	log.Infof("%v: failed to bring %v interface DOWN: %v", l.clientID, l.link.Attrs().Name, err)
	//}
}
...
func (l *DHCPLease) release() error {
	log.Infof("%v: releasing lease", l.clientID)
...
func backoffRetry(resendMax time.Duration, f func() (*dhcp4.Packet, error)) (*dhcp4.Packet, error) {
	...
	for {
		...
		log.Infof("retrying in %f seconds", sleepTime.Seconds())
